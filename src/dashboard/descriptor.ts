 ...
 */
const TEXTUAL_DATA_TYPES: ReadonlySet<DataType> = new Set<DataType>([
  'TEXT',
  'VARCHAR',
]);
 ...
 */
export function generateDescriptor(model: DataModel): DashboardDescriptor {
  return {
...
function toEntityView(entity: Entity): EntityView {
  ...
  const filterableAttributes = entity.attributes.map(
