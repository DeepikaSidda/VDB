...
/** The subset of a `pg` Client the connection adapter relies on. */
...
function toConnectivityError(error: unknown): ConnectivityError {
...
export class PgDriver implements DbDriver {
   ...
   */
  private readonly fixedSchemaName?: string;

  constructor(options: { statementTimeoutMs?: number; schemaPrefix?: string; schemaName?: string } = {}) {
  ...
  async connect(
    ...
    return new PgConnection(
      client,
      this.fixedSchemaName ?? uniqueSchemaName(this.schemaPrefix),
    );
  }
}
