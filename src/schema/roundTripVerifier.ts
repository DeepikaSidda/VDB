 ...
 * `jsonb -> JSON`.
 */
const POSTGRES_TO_DATA_TYPE: Record<string, DataType> = (() => {
  const inverse: Record<string, DataType> = {};
  for (const [dataType, pgType] of Object.entries(DATA_TYPE_TO_POSTGRES)) {
    inverse[normalizePgType(pgType)] = dataType as DataType;
  }
  return inverse;
})();
...
function parseIdentList(fragment: string): string[] {
  const idents: string[] = [];
...
const CREATE_TABLE_RE = new RegExp(
  `^CREATE\\s+TABLE\\s+${QUOTED_IDENT}\\s*\\(([\\s\\S]*)\\)\\s*;?\\s*$`,
  'i',
);

const COLUMN_RE = new RegExp(`^${QUOTED_IDENT}\\s+([\\s\\S]+)$`);

const ADD_FK_RE = new RegExp(
  `^ALTER\\s+TABLE\\s+${QUOTED_IDENT}\\s+ADD\\s+CONSTRAINT\\s+${QUOTED_IDENT}\\s+` +
    `FOREIGN\\s+KEY\\s*\\(\\s*${QUOTED_IDENT}\\s*\\)\\s+` +
    `REFERENCES\\s+${QUOTED_IDENT}\\s*\\(\\s*${QUOTED_IDENT}\\s*\\)\\s*;?\\s*$`,
  'i',
);
 ...
 */
function parseCreateTable(sql: string): Entity | undefined {
...
/** A parsed foreign key: which column on which table references what. */
...
function parseAddForeignKey(sql: string): ParsedForeignKey | undefined {
 ...
 */
function inferIsJoinEntity(entity: Entity): boolean {
 ...
 */
export function parseDDL(ddl: MigrationScript): DataModel {
...
export function relationshipEdges(
  model: DataModel,
): { source: string; target: string; attribute: string }[] {
  const seen = new Set<string>();
  const edges: { source: string; target: string; attribute: string }[] = [];
 ...
 */
function deriveRelationships(entities: Entity[]): Relationship[] {
...
export function constraintProjection(model: DataModel): ConstraintProjection {
    ...
    for (const attribute of entity.attributes) {
      ...
      for (const constraint of attribute.constraints) {
        if (constraint.kind === 'NOT_NULL') {
          ...
          foreignKeys.add(
            `${col}\u0000${constraint.references.entity}\u0000${constraint.references.attribute}`,
          );
        }
      }
    }
...
// ---------------------------------------------------------------------------
// verify — deploy gate (task 4.2; interface + stub only)
// ---------------------------------------------------------------------------
...
export type RoundTripDiff = {
  message: string;
  entities?: { added: string[]; lost: string[]; altered: string[] };
  relationships?: { added: string[]; lost: string[]; altered: string[] };
  constraints?: { added: string[]; lost: string[]; altered: string[] };
};
...
function entitySignatureMap(model: DataModel): Map<string, string> {
...
function diffEntities(
  source: DataModel,
  parsed: DataModel,
): { added: string[]; lost: string[]; altered: string[] } {
...
function labelColumn(kind: string, column: string): string {
...
function labelForeignKey(fk: string): string {
...
function diffConstraints(
  ...
  for (const [entity, sourcePk] of sourceProj.primaryKeys) {
    ...
    if (sourcePk.join('\u0000') !== parsedPk.join('\u0000')) {
      altered.push(
        `PRIMARY_KEY ${entity}: [${sourcePk.join(', ')}] != [${parsedPk.join(
          ', ',
        )}]`,
      );
    }
...
export function verify(
  ...
  const diff: RoundTripDiff = { message: '' };
  const reasons: string[] = [];

  if (!countMatches) {
    reasons.push(
      `table count ${parsed.entities.length} != entity count ${source.entities.length}`,
    );
  }
...
function summarizeGroup(
  label: string,
  diff: { added: string[]; lost: string[]; altered: string[] },
): string {
  const parts: string[] = [];
 ...
 */
export class DefaultRoundTripVerifier implements RoundTripVerifier {
  parseDDL(ddl: MigrationScript): DataModel {
    return parseDDL(ddl);
  }
