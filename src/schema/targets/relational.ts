 ...
 */
export const DATA_TYPE_TO_POSTGRES: Record<DataType, string> = {
  UUID: 'uuid',
  TEXT: 'text',
  VARCHAR: 'varchar(255)',
  INTEGER: 'integer',
 ...
 */
export function mapDataType(dataType: string): string | undefined {
  return (DATA_TYPE_TO_POSTGRES as Record<string, string | undefined>)[
    dataType
  ];
}
...
export type SchemaGenError =
  // Req 3.7 — a relationship references an entity not defined in the model.
  | {
  ...
  // Req 3.8 — a column's data type cannot be mapped to a PostgreSQL type.
  | {
  ...
  // Req 3.9 — the FK dependency graph has a cycle with no valid ordering.
  | {
  ...
  // Req 13.4 — the configured target is not one of the supported targets.
  | {
...
export function foreignKeyColumns(entity: Entity): ForeignKeyColumn[] {
  const fks: ForeignKeyColumn[] = [];
...
function buildColumnDefinition(attribute: Attribute): string {
 ...
 */
function buildCreateTable(entity: Entity): DdlStatement {
  const lines: string[] = entity.attributes.map(buildColumnDefinition);
 ...
 */
function buildForeignKeyStatements(entities: Entity[]): DdlStatement[] {
  ...
  for (const entity of entities) {
    for (const fk of foreignKeyColumns(entity)) {
      ...
      const sql =
        `ALTER TABLE ${quoteIdent(entity.name)} ` +
 ...
 */
function buildForeignKeyIndexStatements(entities: Entity[]): DdlStatement[] {
  const statements: DdlStatement[] = [];
  for (const entity of entities) {
    for (const fk of foreignKeyColumns(entity)) {
 ...
 * resolve.
 */
export function findUndefinedEntity(
  model: DataModel,
): SchemaGenError | undefined {
    ...
    for (const fk of foreignKeyColumns(entity)) {
      if (!defined.has(fk.references.entity)) {
        return {
          kind: 'UNDEFINED_ENTITY',
          message:
            `Foreign key "${entity.name}.${fk.attribute.name}" references ` +
 ...
 */
export function findUnmappableType(
  model: DataModel,
): SchemaGenError | undefined {
  for (const entity of model.entities) {
    for (const attribute of entity.attributes) {
      if (mapDataType(attribute.dataType) === undefined) {
        return {
          kind: 'UNMAPPABLE_TYPE',
          message:
            `Column "${entity.name}.${attribute.name}" has data type ` +
 ...
 */
export function topologicallyOrderEntities(
  model: DataModel,
): Result<Entity[], SchemaGenError> {
  ...
  while (ordered.length < entities.length) {
    ...
    const ready = entities.filter(
      (e) => !placed.has(e.name) && indegree.get(e.name) === 0,
    );
    if (ready.length === 0) {
  ...
  if (ordered.length < entities.length) {
    ...
    return err({
      kind: 'CYCLIC_DEPENDENCY',
      message:
        `Foreign-key dependencies form a cycle with no valid ordering ` +
