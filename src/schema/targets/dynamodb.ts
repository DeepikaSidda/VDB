...
export type UnrepresentedElement =
  ...
  | { kind: 'UNIQUE'; entity: string; attribute: string; reason: string }
  | { kind: 'NOT_NULL'; entity: string; attribute: string; reason: string }
  | {
      kind: 'FORMAT';
      entity: string;
      attribute: string;
 ...
 */
function dynamoKeyType(dataType: DataType): 'S' | 'N' | 'B' {
  switch (dataType) {
    case 'INTEGER':
    case 'BIGINT':
    case 'NUMERIC':
...
// ---------------------------------------------------------------------------
// Table-design generation (Req 13.2)
// ---------------------------------------------------------------------------

type DynamoKeySchemaEntry = { AttributeName: string; KeyType: 'HASH' | 'RANGE' };
type DynamoAttributeDefinition = {
 ...
 */
function buildTableDefinition(entity: Entity): DdlStatement {
  const keySchema: DynamoKeySchemaEntry[] = [];
  const attributeDefinitions: DynamoAttributeDefinition[] = [];

  const typeOf = (name: string): DataType | undefined =>
    entity.attributes.find((a) => a.name === name)?.dataType;
 ...
 */
function collectUnrepresented(model: DataModel): UnrepresentedElement[] {
  const report: UnrepresentedElement[] = [];
  ...
  for (const entity of model.entities) {
    ...
    if (entity.primaryKey.length > 2) {
      report.push({
        ...
        reason:
          `DynamoDB key schema supports at most a partition key and a sort ` +
        ...
        switch (constraint.kind) {
          ...
          case 'FOREIGN_KEY':
            report.push({
              ...
              reason:
                `DynamoDB does not enforce foreign-key references; ` +
          ...
          case 'UNIQUE':
            report.push({
              ...
              attribute: attribute.name,
              reason:
                `DynamoDB enforces uniqueness only on the primary key; the ` +
          ...
          case 'NOT_NULL':
            report.push({
              ...
              attribute: attribute.name,
              reason:
                `DynamoDB requires only key attributes; the not-null ` +
          ...
          case 'FORMAT':
            report.push({
              ...
              reason:
                `DynamoDB does not enforce value formats; the ` +
                `${constraint.format} format constraint on ` +
                `"${entity.name}.${attribute.name}" is not represented.`,
            });
            break;
          case 'RANGE': {
            const element: UnrepresentedElement = {
              kind: 'RANGE',
              entity: entity.name,
              attribute: attribute.name,
              reason:
                `DynamoDB does not enforce value ranges; the range ` +
                `constraint on "${entity.name}.${attribute.name}" is not ` +
                `represented.`,
            };
            if (constraint.min !== undefined) element.min = constraint.min;
            if (constraint.max !== undefined) element.max = constraint.max;
            report.push(element);
            break;
          }
          default: {
  ...
  for (const rel of model.relationships) {
    report.push({
      ...
      reason:
        `DynamoDB has no cross-table relationships; the ${rel.cardinality} ` +
 ...
 */
export function generateDynamoDbDesign(
  model: DataModel,
): Result<DynamoDbDesign, SchemaGenError> {
  const statements = model.entities.map(buildTableDefinition);
  const script: MigrationScript = { target: 'DYNAMODB', statements };
 ...
 */
export function generateDynamoDb(
  model: DataModel,
 ...
 */
export const dynamoDbProjection: TargetProjection = {
  target: 'DYNAMODB',
