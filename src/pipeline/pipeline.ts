...
import {
  Orchestrator,
  type ApiGeneratorPort,
  type Clock as OrchestratorClock,
...
import {
  TransactionalProvisioner,
  type Provisioner,
} from '../provisioner/provisioner.js';
import { type Clock as ProvisionerClock, systemClock } from '../provisioner/driver.js';
 ...
 */
export const LOCAL_DEPLOYMENT_TARGET: DeploymentTarget = {
  kind: 'POSTGRES',
  connection: {
    host: 'localhost',
    port: 5432,
...
function generateSchemaName(prefix = 'gen'): string {
 ...
 */
function topologicalEntityOrder(model: DataModel): string[] {
...
// ---------------------------------------------------------------------------
// Live backend produced by a successful run
// ---------------------------------------------------------------------------
 ...
 */
class ModelingAdapter implements ModelingPort {
  constructor(
    private readonly engine: ModelingEngine,
    private readonly documentParser: DocumentParser,
    private readonly makeImportAnalyzer: (engine: 'postgres' | 'mysql') => ImportAnalyzer,
    private readonly seedRef: { seed?: SeedData },
  ) {}

  async infer(input: JobInput): PortResult<DataModel> {
    switch (input.kind) {
      case 'PROMPT':
      ...
      default: {
        ...
        void _exhaustive;
        return err<PortError>({ message: 'Unsupported input kind' });
      }
    }
  }

  /** DOCUMENT path: extract records (CSV/Excel/PDF) then infer a model. */
  private async fromDocument(
    input: Extract<JobInput, { kind: 'DOCUMENT' }>,
  ): PortResult<DataModel> {
    ...
    try {
      if (isPdf(doc.name, doc.format, doc.contentType)) {
        if (bytes === undefined) {
          return err<PortError>({ message: 'PDF upload must be base64-encoded' });
        }
        ...
        if (sheets.length === 0) {
          return err<PortError>({ message: 'No records were found in the workbook.' });
        }
        if (sheets.length > 1) {
        ...
        if (!parsed.ok) {
          return err<PortError>({ message: parsed.error.message, detail: parsed.error });
        }
  ...
  private async fromImport(
    input: Extract<JobInput, { kind: 'IMPORT' }>,
  ): PortResult<DataModel> {
    const analyzer = this.makeImportAnalyzer(input.engine);
    const result = await analyzer.importSchema(input.connection);
    if (!result.ok) {
      return err<PortError>({ message: result.error.message, detail: result.error });
    }
  ...
  private async fromModeling(
    promise: Promise<{ ok: true; value: DataModel } | { ok: false; error: { message: string } }>,
  ): PortResult<DataModel> {
   ...
   */
  private async fromModelingAndSeed(
    promise: Promise<
      | { ok: true; value: { model: DataModel; seed: SeedData } }
...
function isPdf(name: string, format?: string, contentType?: string): boolean {
  return (
...
function isExcel(name: string, format?: string, contentType?: string): boolean {
 ...
 */
class RefinementAdapter implements RefinementPort {
  constructor(private readonly engine: RefinementEngine) {}

  async refine(model: DataModel): PortResult<DataModel> {
 ...
 */
class SchemaGeneratorAdapter implements SchemaGeneratorPort {
  async generate(model: DataModel): PortResult<MigrationScript> {
 ...
 */
class RoundTripVerifierAdapter implements RoundTripVerifierPort {
  constructor(private readonly verifier: DefaultRoundTripVerifier) {}
 ...
 */
class ProvisionerAdapter implements ProvisionerPort {
  constructor(
    private readonly provisioner: Provisioner,
    private readonly target: DeploymentTarget,
  ) {}

  async apply(migration: MigrationScript): PortResult<void> {
 ...
 */
class ApiGeneratorAdapter implements ApiGeneratorPort {
  constructor(
    private readonly store: RecordStore,
    private readonly auth: AuthService,
    private readonly backendRef: { backend?: Backend },
    private readonly seedRef: { seed?: SeedData },
  ) {}

  async generate(model: DataModel): PortResult<void> {
 ...
 */
class BackendCompensation implements CompensationPort {
  constructor(
    private readonly store: RecordStore,
    private readonly backendRef: { backend?: Backend },
  ) {}

  async compensate(job: GenerationJob): Promise<void> {
...
export class GenerationPipeline {
  ...
  async run(input: JobInput): Promise<PipelineRunResult> {
    ...
    const makeImportAnalyzer = (engine: 'postgres' | 'mysql'): ImportAnalyzer =>
      new SourceImportAnalyzer(engine === 'mysql' ? new MySqlSource() : new PgSource());

    const modelingAdapter = new ModelingAdapter(
      modelingEngine,
      documentParser,
      makeImportAnalyzer,
      seedRef,
    ...
    const deployedSchema =
      this.deps.makeProvisioner !== undefined ? generateSchemaName() : undefined;
    const provisioner =
      deployedSchema !== undefined && this.deps.makeProvisioner !== undefined
        ? this.deps.makeProvisioner(deployedSchema)
        : this.deps.provisioner ??
          new TransactionalProvisioner(
            new InMemoryDriver(),
            this.deps.provisionerClock ?? systemClock,
          );
    ...
    let dataPersistence: PipelineRunResult['dataPersistence'];
    if (
      job.status === 'deployed' &&
      ...
      try {
        const detail = await this.deps.persistSeed(
          deployedSchema,
          backend.model,
          seedRef.seed,
        );
 ...
 * client by hand.
 */
export function createStubPipeline(
  stub: ConstructorParameters<typeof StubLlmClient>[0],
