...
/** The subset of a `pg` Client this reader relies on. */
 ...
 */
export async function readPostgresRows(
  creds: DbCredentials,
  schema: string,
    ...
    for (const table of tables) {
      try {
        const res = await client.query(
          `SELECT * FROM ${quoteIdent(schema)}.${quoteIdent(table)} LIMIT ${MAX_ROWS_PER_TABLE}`,
        );
