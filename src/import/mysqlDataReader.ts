...
/** The subset of a `mysql2/promise` connection this reader relies on. */
    ...
    for (const table of tables) {
      try {
        const [rows] = await conn.query(
          `SELECT * FROM ${quoteIdent(table)} LIMIT ${MAX_ROWS_PER_TABLE}`,
        );
        out.set(table, rows as Record<string, unknown>[]);
      } catch {
