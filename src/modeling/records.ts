...
import { type Result, err, isErr, ok } from '../model/result.js';
...
// ---------------------------------------------------------------------------
// Field type inference (Requirement 10.3)
// ---------------------------------------------------------------------------

/**
 * The intermediate value categories a single cell can fall into. They are
 * reconciled across a column's sample values into one supported `DataType`.
 */
type ValueCategory =
  | 'BOOLEAN' | 'INTEGER' | 'NUMERIC' | 'DATE' | 'TIMESTAMP' | 'TEXT';
 ...
 */
function classifyValue(value: unknown): ValueCategory {
 ...
 * falls back to `TEXT`.
 */
export function inferFieldType(values: readonly unknown[]): DataType {
  ...
  if (categories.size === 1) {
    return [...categories][0] satisfies DataType;
  }
 ...
 */
export function collectFieldNames(records: readonly ExtractedRecord[]): string[] {
...
function partitionSignature(
  ...
  field: string,
): { signature: string; distinctClasses: number } {
...
export function detectRepeatingGroups(
  ...
  const clusters = new Map<
    string,
    { fields: string[]; distinctClasses: number }
  >();
 ...
 * prefix.
 */
function deriveGroupName(fields: readonly string[]): string {
...
/**
 * Binds an extracted group entity (by its final name) to the source field
 * names it was built from, so the seeding pass can reconstruct each group's
 * tuples and link the main rows to them via the inferred foreign key.
 */
type GroupBinding = { entityName: string; fields: string[] };

/**
 * The product of {@link assembleCandidate}: the raw candidate model plus the
 * names needed to seed it — the main entity's name and the field→entity
 * bindings for every extracted group.
 */
...
function assembleCandidate(
  records: readonly ExtractedRecord[],
  fieldNames: readonly string[],
  groups: readonly RepeatingGroup[],
): CandidateAssembly {
  ...
  const mainAttributes: RawCandidateAttribute[] = fieldNames
    .filter((field) => !extractedFields.has(field))
    .map((field) => rawAttribute(field, records));

  const entities: RawCandidateEntity[] = [
    { name: mainName, attributes: mainAttributes },
  ];
  const relationships: RawCandidateModel['relationships'] = [];
...
// ---------------------------------------------------------------------------
// Top-level decomposition (Requirements 10.2, 10.3)
// ---------------------------------------------------------------------------
...
export type ModelAndSeed = { model: DataModel; seed: SeedData };
 ...
 */
function coerceToType(value: unknown, dataType: DataType): unknown {
  if (isBlank(value)) {
    return undefined;
  }
  switch (dataType) {
...
function buildSeed(
  ...
  const fieldType = new Map<string, DataType>();
  for (const field of fieldNames) {
  ...
  type GroupPlan = {
    ...
    pkColumn: string;
    fkColumn: string;
    rowBySig: Map<string, unknown>; // signature -> assigned id
    rows: SeedRecord[];
  };
  ...
  for (const binding of assembly.groupBindings) {
    ...
    const fkAttr = mainEntity.attributes.find((a) =>
      a.constraints.some(
        (c) => c.kind === 'FOREIGN_KEY' && c.references.entity === binding.entityName,
      ),
    );
    if (fkAttr === undefined) {
      continue;
  ...
  const mainRows: SeedRecord[] = [];
  for (const record of records) {
    const mainRow: SeedRecord = { [mainPk]: randomUUID() };
    ...
    for (const plan of groupPlans) {
      ...
      if (id === undefined) {
        ...
        const groupRow: SeedRecord = { [plan.pkColumn]: id };
        for (const field of plan.fields) {
          const v = coerceToType(record[field], fieldType.get(field) ?? 'TEXT');
          if (v !== undefined) {
            groupRow[field] = v;
 ...
 */
export function inferModelAndSeedFromRecords(
  records: readonly ExtractedRecord[],
): Result<ModelAndSeed, ModelingError> {
  if (records.length === 0) {
    return err({
      kind: 'NO_DATA_MODEL',
      message:
        'No Data_Model could be derived from the document: no records were provided.',
    });
  }
  ...
  if (fieldNames.length === 0) {
    return err({
      kind: 'NO_DATA_MODEL',
      message:
        'No Data_Model could be derived from the document: the records contain no named fields.',
    });
  }
 ...
 */
export function inferModelFromRecords(
  records: readonly ExtractedRecord[],
): Result<DataModel, ModelingError> {
...
// ---------------------------------------------------------------------------
// Multi-sheet workbooks (each sheet -> its own entity; FKs inferred by name)
// ---------------------------------------------------------------------------

/** One worksheet's records, tagged with the sheet (table) name. */
export type SheetInput = { name: string; records: readonly ExtractedRecord[] };

/** True when `column` looks like a foreign key by naming (`x_id` / `xId`). */
function looksLikeForeignKeyName(column: string): boolean {
  if (/^id$/i.test(column)) {
    return false;
  }
...
function foreignKeyBase(column: string): string {
  return column.replace(/(_id|Id)$/, '');
}

/** Find an entity whose name matches `base` (exact / singular / plural, ci). */
function matchEntityName(base: string, entityNames: readonly string[]): string | undefined {
 ...
 */
export function inferModelAndSeedFromSheets(
  ...
  const synthesizedPk = new Map<string, string>(); // entity -> synthesized pk col
  ...
  const model: DataModel = { entities: enriched.entities, relationships };
  ...
  for (const entityName of order) {
    ...
    const typeOf = new Map<string, DataType>(entity.attributes.map((a) => [a.name, a.dataType]));
    const rows: SeedRecord[] = se.records.map((record) => {
      const row: SeedRecord = {};
...
function topoOrder(model: DataModel): string[] {
  ...
  for (const n of names) if (!placed.has(n)) ordered.push(n);
  return ordered;
}
