...
const NUMERIC_TYPES: ReadonlySet<DataType> = new Set<DataType>([
  'INTEGER', 'BIGINT', 'NUMERIC',
]);

/** Text-ish `DataType`s eligible for an EMAIL format constraint. */
const TEXTUAL_TYPES: ReadonlySet<DataType> = new Set<DataType>([
  'TEXT', 'VARCHAR',
]);
 ...
 */
const UNIQUE_TOKENS: ReadonlySet<string> = new Set<string>([
 ...
 */
const LOWER_BOUND_ZERO_TOKENS: ReadonlySet<string> = new Set<string>([
 ...
 */
const AMBIGUOUS_IDENTIFIER_TOKENS: ReadonlySet<string> = new Set<string>([
...
function hasAnyToken(tokens: string[], vocab: ReadonlySet<string>): boolean {
...
function isForeignKey(attribute: Attribute): boolean {
...
function buildHintLookup(
  raw: RawCandidateModel | undefined,
): Map<string, Map<string, RawCandidateAttribute>> {
  const byEntity = new Map<string, Map<string, RawCandidateAttribute>>();
  if (!raw || !Array.isArray(raw.entities)) {
    return byEntity;
  }
  for (const rawEntity of raw.entities) {
    const entityName =
      typeof rawEntity?.name === 'string' ? rawEntity.name.trim() : '';
    if (entityName.length === 0) {
      continue;
    }
    const attrs = new Map<string, RawCandidateAttribute>();
    for (const rawAttr of rawEntity.attributes ?? []) {
      const attrName =
        typeof rawAttr?.name === 'string' ? rawAttr.name.trim() : '';
 ...
 */
function inferAttributeConstraints(
  attribute: Attribute,
  isPrimaryKey: boolean,
  isJoinEntity: boolean,
  hint: RawCandidateAttribute | undefined,
): Attribute {
  ...
  const constraints: AttributeConstraint[] = [...attribute.constraints];
  ...
  const confidentUnique =
    hint?.unique === true || hasAnyToken(tokens, UNIQUE_TOKENS);
  ...
  if (
    TEXTUAL_TYPES.has(attribute.dataType) &&
  ...
  if (
    NUMERIC_TYPES.has(attribute.dataType) &&
...
function uniqueAttrName(base: string, taken: Set<string>): string {
  if (!taken.has(base)) {
    taken.add(base);
    return base;
  }
 ...
 */
function addRelationshipForeignKeys(
  model: DataModel,
  ...
  for (const rel of model.relationships) {
    ...
    const takenAttrNames = new Set<string>(
      dependent.attributes.map((a) => a.name),
    );
    ...
    for (const pkAttrName of target.primaryKey) {
      ...
      const alreadyReferenced = dependent.attributes.some((a) =>
        a.constraints.some(
          (c) =>
            c.kind === 'FOREIGN_KEY' &&
      ...
      const columnName = uniqueAttrName(
        `${rel.target}_${pkAttrName}`,
        takenAttrNames,
      );
...
export function inferConstraints(
  ...
  const entities: Entity[] = model.entities.map((entity) => {
    ...
    const attributes = entity.attributes.map((attribute) =>
      inferAttributeConstraints(
        attribute,
        pkSet.has(attribute.name),
        entity.isJoinEntity,
        entityHints?.get(attribute.name),
      ),
    );
  ...
  const byName = new Map<string, Entity>(entities.map((e) => [e.name, e]));
