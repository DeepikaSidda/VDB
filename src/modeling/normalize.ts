...
import type { RawCandidateEntity, RawCandidateModel } from './llmClient.js';
 ...
 */
const DATA_TYPE_ALIASES: Readonly<Record<string, DataType>> = {
  // UUID
  uuid: 'UUID', guid: 'UUID',
 ...
 */
const ONE_TO_MANY_LABELS = new Set<string>([
 ...
 */
export function normalizeDataType(raw: string | undefined): DataType {
...
export function normalizeCardinality(
  ...
  const key =
    typeof raw === 'string' ? raw.toLowerCase().replace(/[^a-z0-9]/g, '') : '';
 ...
 */
function normalizeEntityBody(rawEntity: RawCandidateEntity): {
 ...
 */
function buildJoinEntity(
  source: Entity,
  target: Entity,
  takenEntityNames: Set<string>,
): Entity {
  ...
  const addForeignKeys = (endpoint: Entity): void => {
    for (const pkAttrName of endpoint.primaryKey) {
      const referenced = endpoint.attributes.find(
        (a) => a.name === pkAttrName,
      );
      const columnName = uniqueName(
        `${endpoint.name}_${pkAttrName}`,
        takenAttrNames,
      );
      attributes.push({
        name: columnName,
...
export function normalizeCandidate(
  ...
  if (entities.length === 0) {
    return err({
      kind: 'NO_DATA_MODEL',
      message:
        'No Data_Model could be derived from the description: no entity could be inferred.',
    });
  }
