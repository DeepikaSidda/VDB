...
type Domain = {
  /** Keywords (lowercased) that select this domain. */
  match: string[];
  model: RawCandidateModel;
};

const id = { name: 'id', dataType: 'UUID', required: true } as const;
const name = { name: 'name', dataType: 'TEXT', required: true } as const;
const email = { name: 'email', dataType: 'TEXT', unique: true, required: true } as const;
 ...
 */
export function heuristicCandidate(prompt: string): RawCandidateModel {
  ...
  const capitalized = Array.from(
    new Set(
      (prompt.match(/\b[A-Z][a-zA-Z]{2,}\b/g) ?? [])
        .map((w) => titleCase(w))
        // Drop common leading verbs/filler that aren't entities.
        .filter((w) => !['Build', 'Create', 'Make', 'Generate', 'System', 'App', 'The'].includes(w)),
    ),
  ).slice(0, 5);
