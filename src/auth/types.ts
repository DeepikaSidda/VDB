...
export type AuthError =
  | { kind: 'MISSING_CREDENTIAL'; credential: 'identifier' | 'password' } // Req 6.7
  | { kind: 'INVALID_CREDENTIALS' }                                       // Req 6.3
  | { kind: 'IDENTIFIER_IN_USE'; identifier: string }                     // Req 6.8
  | { kind: 'INVALID_TOKEN'; reason: 'MISSING' | 'MALFORMED' | 'EXPIRED' } // Req 6.4 (Task 8.2)
  | { kind: 'INSUFFICIENT_ROLE'; required: Role };                        // Req 6.5 (Task 8.2)
