...
import { CONNECT_TIMEOUT_MS, SourceAuthenticationError, SourceConnectionTimeoutError, systemClock, } from './sourceDriver.js';
// ---------------------------------------------------------------------------
 ...
 */
const SOURCE_TYPE_TO_DATA_TYPE = {
    // UUID
    uuid: 'UUID',
    // TEXT-ish
    text: 'TEXT',
 ...
 */
export function mapSourceType(rawType) {
...
const FALLBACK_DATA_TYPE = 'TEXT';
/** Build a single Data_Model attribute from a source column. */
function buildAttribute(table, column, notExtracted) {
    const constraints = [];
 ...
 */
function schemaToDataModel(schema) {
    const notExtracted = [];
    const entities = [];
    const relationships = [];
    for (const table of schema.tables) {
        const attributes = table.columns.map((column) => buildAttribute(table, column, notExtracted));
        entities.push({
            name: table.name,
            attributes,
 ...
 */
export class SourceImportAnalyzer {
    driver;
    clock;
    constructor(driver, clock = systemClock) {
        this.driver = driver;
        this.clock = clock;
    }
    async importSchema(creds) {
 ...
 * `id` primary key.
 */
function missingPrimaryKeySuggestions(model) {
    const suggestions = [];
 ...
 */
function looksLikeForeignKey(columnName) {
...
function foreignKeyBaseName(columnName) {
 ...
 * likely target.
 */
function missingForeignKeySuggestions(model) {
    const suggestions = [];
    ...
    for (const entity of model.entities) {
        for (const attribute of entity.attributes) {
            ...
            suggestions.push({
                ...
                issue: `Column "${entity.name}.${attribute.name}" is named like a foreign ` +
                    `key but has no foreign-key constraint.`,
                proposedChange: `Add a foreign-key constraint from "${entity.name}.${attribute.name}" ` +
 ...
 */
function findCandidateEntity(base, entityNames) {
 ...
 */
function normalizationSuggestions(model) {
    const suggestions = [];
    const entityNames = model.entities.map((e) => e.name);
    for (const entity of model.entities) {
...
function repeatingGroupBase(columnName) {
...
function repeatingGroupSuggestions(entity) {
    ...
    for (const [base, members] of groups) {
        if (members.length >= 2) {
            suggestions.push({
                kind: 'NORMALIZATION',
                element: { kind: 'ENTITY', entity: entity.name },
                issue: `Entity "${entity.name}" has a repeating group of columns ` +
                    `(${members.join(', ')}), which violates first normal form.`,
                proposedChange: `Extract the repeating "${base}" values into a separate "${base}" ` +
...
function transitiveDependencySuggestions(entity, entityNames) {
    ...
    for (const [prefix, members] of groups) {
        ...
        if (members.length >= 2 && hasNonIdColumn && (hasIdColumn || matchesEntity)) {
            suggestions.push({
                kind: 'NORMALIZATION',
                element: { kind: 'ENTITY', entity: entity.name },
                issue: `Columns (${members.join(', ')}) in "${entity.name}" appear to ` +
                    `depend on "${prefix}" rather than the entity's primary key, which ` +
                    `violates third normal form.`,
                proposedChange: `Move the "${prefix}_*" attributes into a separate "${prefix}" ` +
