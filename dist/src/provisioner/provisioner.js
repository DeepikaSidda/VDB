 ...
 * driver.
 */
import { APPLY_CEILING_MS, CONNECT_TIMEOUT_MS, ConnectivityError, systemClock, } from './driver.js';
/**
 ...
 */
export class TransactionalProvisioner {
    driver;
    clock;
    ...
    async apply(script, target) {
        ...
        if (script.target !== target.kind) {
            return {
                status: 'failed',
                reason: `target mismatch: migration was generated for ${script.target} ` +
     ...
     */
    assertWithinCeiling(applyStart) {
        if (this.clock.now() - applyStart > APPLY_CEILING_MS) {
            throw new Error(`migration exceeded the ${APPLY_CEILING_MS}ms apply ceiling`);
        }
    }
}
// ---------------------------------------------------------------------------
