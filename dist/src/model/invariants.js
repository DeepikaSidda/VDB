...
const ALLOWED_CARDINALITIES = [
    'ONE_TO_ONE', 'ONE_TO_MANY', 'MANY_TO_MANY',
];
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
function entityNameSet(model) {
    return new Set(model.entities.map((e) => e.name));
}
function entityByName(model) {
    return new Map(model.entities.map((e) => [e.name, e]));
}
function foreignKeysOf(entity) {
    const fks = [];
 ...
 */
function referencesPrimaryKeyOf(entity, targetName, target) {
    const pk = new Set(target?.primaryKey ?? []);
    return foreignKeysOf(entity).some((fk) => fk.references.entity === targetName &&
...
export function checkI2(model) {
    const supported = new Set(SUPPORTED_DATA_TYPES);
    const violations = [];
    for (const entity of model.entities) {
        for (const attribute of entity.attributes) {
            if (!supported.has(attribute.dataType)) {
                violations.push({
                    invariant: 'I2',
                    message: `Attribute "${entity.name}.${attribute.name}" has unsupported data type "${String(attribute.dataType)}".`,
                    entity: entity.name,
...
export function checkI3(model) {
    const allowed = new Set(ALLOWED_CARDINALITIES);
    const violations = [];
    for (const rel of model.relationships) {
        if (!allowed.has(rel.cardinality)) {
            violations.push({
                invariant: 'I3',
                message: `Relationship "${rel.source}" -> "${rel.target}" has invalid cardinality "${String(rel.cardinality)}".`,
...
export function checkI4(model) {
    ...
    for (const rel of model.relationships) {
        ...
        const hasJoinEntity = model.entities.some((entity) => entity.isJoinEntity &&
 ...
 */
export function validateInvariant(model, id) {
