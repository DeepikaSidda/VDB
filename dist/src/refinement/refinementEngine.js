 ...
 */
function entityStructurallyEqual(a, b) {
...
function refExists(model, ref) {
    switch (ref.kind) {
        ...
        case 'RELATIONSHIP':
            return model.relationships.some((r) => r.source === ref.source && r.target === ref.target);
    }
}
/** Map an invariant violation to the element it concerns, for conflict reporting. */
function violationToElement(v) {
    switch (v.invariant) {
        case 'I1':
 ...
 */
function timestampsQuestion(entity, id) {
    const addition = {
        attributes: [
            {
                entity: entity.name,
 ...
 * (Req 8.4).
 */
function softDeleteQuestion(entity, id) {
    const addition = {
        attributes: [
            {
                entity: entity.name,
 ...
 * mutating the model).
 */
function relationshipQuestion(rel, id) {
    return {
        id,
 ...
 */
export function deriveQuestions(model) {
    const questions = [];
    let seq = 0;
    ...
    return questions
        .filter((q) => q.groundedIn.length > 0 &&
 ...
 */
function applyAddition(working, addition, questionId, optionId) {
    ...
    for (const newEntity of addition.entities ?? []) {
        ...
        if (existing) {
            ...
            working.entities.push(JSON.parse(JSON.stringify(newEntity)));
        }
    }
    ...
    for (const { entity, attribute } of addition.attributes ?? []) {
        ...
        if (existingAttr) {
            ...
            ent.attributes.push(JSON.parse(JSON.stringify(attribute)));
        }
    }
    ...
    for (const newRel of addition.relationships ?? []) {
        ...
        const existing = working.relationships.find((r) => r.source === newRel.source && r.target === newRel.target);
        if (existing) {
 ...
 * I1–I6.
 */
export function applyAnswers(model, answers) {
    ...
    for (const answer of answers) {
        for (const option of answer.selectedOptions) {
            ...
            const conflict = applyAddition(working, option.effect.addition, answer.questionId, option.id);
 ...
 */
export function selectOption(question, optionId) {
...
//# sourceMappingURL=refinementEngine.js.map