...
export const SUPPORTED_FORMATS = ['CSV', 'Excel', 'PDF'];
// ---------------------------------------------------------------------------
// Format detection
// ---------------------------------------------------------------------------
/**
...
export function detectFormat(file) {
    ...
    if (hint) {
        if (hint === 'csv')
            return 'CSV';
        if (hint === 'excel' || hint === 'xlsx' || hint === 'xls')
            return 'EXCEL';
        if (hint === 'pdf')
            return 'PDF';
        ...
        switch (format) {
            ...
            case 'EXCEL': {
                if (!this.excelExtractor) {
                    throw new Error('no Excel extractor is configured for this Document_Parser');
                }
                return this.excelExtractor(file);
            }
            case 'PDF': {
                if (!this.pdfExtractor) {
                    throw new Error('no PDF extractor is configured for this Document_Parser');
                }
