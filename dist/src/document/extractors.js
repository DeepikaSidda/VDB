...
function toWorkbookInput(content) {
 ...
 */
export const xlsxExtractor = (file) => {
 ...
 */
export function xlsxSheets(file) {
    const input = toWorkbookInput(file.content);
    const workbook = XLSX.read(input.data, { type: input.type });
    const out = [];
    const taken = new Set();
 ...
 */
export async function extractPdfRecords(bytes) {
 ...
 */
export function pdfTextToRecords(text) {
    const lines = text
        .split(/\r?\n/)
        .map((l) => l.trim())
        .filter((l) => l.length > 0);
