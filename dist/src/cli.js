...
async function main() {
    ...
    if (prompt.length === 0) {
        process.stderr.write('Usage: node dist/src/cli.js "<domain description>"\n');
        process.exitCode = 2;
        return;
    }
    const { pipeline, config } = createPipelineFromEnv(process.env, {
        observer: {
            onStageTransition: (e) => process.stdout.write(`  → ${e.stage}\n`),
            onNotice: (e) => process.stdout.write(`  ! ${e.message}\n`),
        },
    });
    process.stdout.write(`LLM: ${config.llm.provider} | deploy target: ${config.deployment.kind}\n`);
    ...
    if (job.status !== 'deployed' || backend === undefined) {
        process.stderr.write(`\nGeneration failed at stage ${job.failure?.stage ?? job.currentStage}: ${job.failure?.reason ?? 'unknown error'}\n`);
        process.exitCode = 1;
        return;
    }
    ...
    process.stdout.write(`  Dashboard views: ${backend.dashboard.entities
        .map((e) => e.entityName)
        .join(', ')}\n`);
}
main().catch((error) => {
